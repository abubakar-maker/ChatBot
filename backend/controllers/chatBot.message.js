...
export const Message = async (req, res) => {
  try {
    ...
    const nameMatch = stringSimilarity.findBestMatch(lowerText, nameQuestions);

    if (nameMatch.bestMatch.rating > 0.5) {
      let botMessage = "You haven't told me your name yet.";
      if (userMemory.name) {
        botMessage = `Your name is ${userMemory.name}`;
      }
      const bot = await Bot.create({ text: botMessage });
      return res.json({ userMessage: user.text, botMessage: bot.text });
    }
    ...
    // =========================
    // MEMORY FEATURE: FAVORITE LANGUAGE
    // =========================

    if (lowerText.includes("my favorite language is")) {
      const lang = text.split("my favorite language is")[1]?.trim();
      if (lang) {
        userMemory.language = lang;
        const botMessage = `${lang} is a great language! I'll remember that.`;
        const bot = await Bot.create({ text: botMessage });
        return res.json({ userMessage: user.text, botMessage: bot.text });
      }
    }

    // Typo-tolerant language questions
    ...
    const langMatch = stringSimilarity.findBestMatch(lowerText, languageQuestions);

    if (langMatch.bestMatch.rating > 0.5) {
      let botMessage = "You haven't told me your favorite language yet.";
      if (userMemory.language) {
        botMessage = `You like ${userMemory.language}`;
      }
      const bot = await Bot.create({ text: botMessage });
      return res.json({ userMessage: user.text, botMessage: bot.text });
    }
    ...
    if (lowerText.includes("weather")) {
      ...
      if (lowerText.includes("in ")) {
        city = lowerText.split("in ")[1].trim();
      }
 ...
 const botData = [
  ...
  },
  {
    ...
    let bestMatch = { rating: 0, response: "" };
